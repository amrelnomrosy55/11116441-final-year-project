# ABLATION (no_clamp): variant of train_v2.py with mask floor changed from
# clamp(min=0.05) to clamp(min=0.0). Everything else is identical to v2.
import os
import sys
# ABLATION: ensure repo root is importable so `dataset_modified` resolves when
# running `python ablation/train_no_clamp.py` from the repo root.
...
# ABLATION (no_clamp): redirected save path to ablation/outputs/ tree.
MODEL_SAVE_PATH = "ablation/outputs/no_clamp.pth"  # New save path to preserve original
...
# -----------------------
# ABLATION (no_clamp): mask floor lowered from 0.05 to 0.0.
class UNetMasker(nn.Module):
    ...
    def forward(self, x):
        ...
        # ABLATION (no_clamp): mask floor relaxed to 0.0 so the network can drive bins to true zero.
        mask = torch.sigmoid(self.dec1(torch.cat([d2, e1], dim=1))).clamp(min=0.0)
...
def main():
    ...
    # ABLATION (no_clamp): also ensure the ablation output dir exists.
    ...
    # ABLATION (no_clamp): variant banner for log readability.
    print(f"[ABLATION] variant: no_clamp")
    ...
    import json
    # ABLATION (no_clamp): per-variant loss history file so baselines are not overwritten.
    loss_path = "ablation/outputs/loss_history_no_clamp.json"
