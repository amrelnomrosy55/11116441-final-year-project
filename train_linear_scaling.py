# ABLATION (linear_scaling): variant of train_v2.py with the log1p input
# scaling removed from the model. The training loss still uses log1p on both
# prediction and target (the ablation targets *input* scaling only, not the
# loss formulation).
import os
import sys
# ABLATION: ensure repo root is importable so `dataset_modified` resolves when
# running `python ablation/train_linear_scaling.py` from the repo root.
...
# ABLATION (linear_scaling): redirected save path to ablation/outputs/ tree.
MODEL_SAVE_PATH = "ablation/outputs/linear_scaling.pth"  # New save path to preserve original
...
# -----------------------
# ABLATION (linear_scaling): encoder now consumes raw magnitudes, not log1p(magnitudes).
class UNetMasker(nn.Module):
    ...
    def forward(self, x):
        ...
        # ABLATION (linear_scaling): log1p scaling removed; encoder receives raw magnitude x.
        # x_log = torch.log1p(x)

        # Encoder passes
        # Added: BatchNorm inserted between conv and activation in each encoder stage
        # ABLATION (linear_scaling): enc1 now takes x instead of x_log.
        e1 = F.leaky_relu(self.bn_e1(self.enc1(x)), 0.2)
...
def main():
    ...
    # ABLATION (linear_scaling): also ensure the ablation output dir exists.
    ...
    # ABLATION (linear_scaling): variant banner for log readability.
    print(f"[ABLATION] variant: linear_scaling")
    ...
    import json
    # ABLATION (linear_scaling): per-variant loss history file so baselines are not overwritten.
    loss_path = "ablation/outputs/loss_history_linear_scaling.json"
