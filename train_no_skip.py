...
class UNetMasker(nn.Module):
    def __init__(self):
        ...
        # ABLATION (no_skip): dec2 input shrinks from 96 (cat[d3,e2]) to 48 (d3 alone).
        # self.dec2 = nn.ConvTranspose2d(96, 24, 4, stride=2, padding=1)     # Input: cat[dec3, e2] = 96
        self.dec2 = nn.ConvTranspose2d(48, 24, 4, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        # ABLATION (no_skip): feed only d4 into dec3 (no concat with e3).
        d3 = F.leaky_relu(self.bn_d3(self.dec3(d4)), 0.2)
        if d3.shape[-2:] != e2.shape[-2:]:
            d3 = F.interpolate(d3, size=e2.shape[-2:], mode="bilinear", align_corners=False)

        # Decoder 2 -> Resize to match e1 (Skip Connection 3)
        # ABLATION (no_skip): feed only d3 into dec2 (no concat with e2).
        d2 = F.leaky_relu(self.bn_d2(self.dec2(d3)), 0.2)
...
def main():
    ...
    # ABLATION (no_skip): also ensure the ablation output dir exists.
    ...
    # ABLATION (no_skip): variant banner for log readability.
    print(f"[ABLATION] variant: no_skip")
    ...
    import json
    # ABLATION (no_skip): per-variant loss history file so baselines are not overwritten.
    loss_path = "ablation/outputs/loss_history_no_skip.json"
