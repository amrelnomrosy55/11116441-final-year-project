# ABLATION (no_bn): variant of train_v2.py with all BatchNorm2d layers removed.
import os
import sys
# ABLATION: ensure repo root is importable so `dataset_modified` resolves when
# running `python ablation/train_no_bn.py` from the repo root.
...
# ABLATION (no_bn): redirected save path to ablation/outputs/ tree.
MODEL_SAVE_PATH = "ablation/outputs/no_bn.pth"  # New save path to preserve original
...
# -----------------------
# ABLATION (no_bn): BatchNorm2d layers removed from encoder and decoder;
# architecture is otherwise identical to v2.
class UNetMasker(nn.Module):
    def __init__(self):
        ...
        # ABLATION (no_bn): encoder BatchNorm layers disabled below.
        ...
        # ABLATION (no_bn): decoder BatchNorm layers disabled below.
    ...
    def forward(self, x):
        ...
        # ABLATION (no_bn): BN wrappers removed from activations.
        e1 = F.leaky_relu(self.enc1(x_log), 0.2)
        e2 = F.leaky_relu(self.enc2(e1), 0.2)
        e3 = F.leaky_relu(self.enc3(e2), 0.2)
        e4 = F.leaky_relu(self.enc4(e3), 0.2)          # NEW encoder stage
        ...
        # ABLATION (no_bn): BN wrappers removed from decoder activations.
        d4 = F.leaky_relu(self.dec4(e4), 0.2)
        if d4.shape[-2:] != e3.shape[-2:]:
            d4 = F.interpolate(d4, size=e3.shape[-2:], mode="bilinear", align_corners=False)

        # Decoder 3 -> Resize to match e2 (Skip Connection 2)
        d3 = F.leaky_relu(self.dec3(torch.cat([d4, e3], dim=1)), 0.2)
        if d3.shape[-2:] != e2.shape[-2:]:
            d3 = F.interpolate(d3, size=e2.shape[-2:], mode="bilinear", align_corners=False)

        # Decoder 2 -> Resize to match e1 (Skip Connection 3)
        d2 = F.leaky_relu(self.dec2(torch.cat([d3, e2], dim=1)), 0.2)
...
def main():
    ...
    # ABLATION (no_bn): also ensure the ablation output dir exists.
    ...
    # ABLATION (no_bn): variant banner for log readability.
    print(f"[ABLATION] variant: no_bn")
    ...
    for epoch in range(EPOCHS):
        ...
        for noisy_mag, clean_mag in dl:
            ...
            optimizer.zero_grad()
            loss.backward()
            # Added: Gradient clipping prevents exploding gradients (safe upper bound = 1.0)
    ...
    import json
    # ABLATION (no_bn): per-variant loss history file so baselines are not overwritten.
    loss_path = "ablation/outputs/loss_history_no_bn.json"
